...
public class MovieManager {
   ...
   /*
    *   변수 => 기본형 / 배열 / 클래스 
    *   Movie : 사용자 정의 테이터형 => 배열 , 형변환도 가능하다 
    *   
    *   초기화 
    *   static 
    *   {
   ...
   public MovieManager()
   {
	   ...
	   {
		   // 정상적으로 수행 => 명령문 
		   // => 예외처리는 모든 문장에 있다 => 생략 => 파일입출력 / 네크워크 / 데이터베이스
		   ...
		   String data=sb.toString(); // 전체 데이터를 모운다 
   ...
   {
	   return (int)(Math.ceil(movies.length/20.0)); //ceil올림 
   }
   ...
   public static void main(String[] args) {
	   ...
	   MovieManager m=new MovieManager();
		/*
		 * int total=m.movieTotalPage(); // 인스턴스변수 / 인스턴스메소드 ==> 99:1
		 * System.out.print("1~"+total+"사이의 페이지 입력:"); int page=scan.nextInt();
		 * 
		 * // 데이터 읽기 Movie[] movie=m.movieListData(page); for(Movie mm:movie) {
		 * System.out.println(mm.getRank()+"."+mm.getTitle()); }
		 */
	   System.out.print("검색어 입력:");
	   String title=scan.next();
	   
	   Movie[] movies=m.movieFindData(title);
	   for(Movie mm:movies) {
		   System.out.println(mm.getRank()+","+mm.getTitle());
	   }
	   System.out.println("=====");
	   System.out.println("상세볼 영화번호:");
